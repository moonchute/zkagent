...
describe("A regex", function () {
  ...
  beforeAll(async () => {
    cir = await wasm_tester(
      path.join(__dirname, "../mocks/regexes/test_repo.circom"),
      {
        include: path.join(__dirname, "../../node_modules"),
        output: path.join(__dirname, "../../build/test_repo"),
        recompile: true,
        verbose: true
      }
    )
  ...
  it("Should generate witness", async () => {
    const input = {
      "msg": textToAsciiArray("\nto:jstinhw/zk-grant-testing <")
    };
    const expectedValue = Array(textToAsciiArray("\nto:").length).fill("0")
      .concat(textToAsciiArray("jstinhw/zk-grant-testing"))
