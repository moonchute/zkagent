...
describe("A regex", function () {
  ...
  beforeAll(async () => {
    cir = await wasm_tester(
      path.join(__dirname, "../mocks/regexes/test_pr_num.circom"),
      {
        include: path.join(__dirname, "../../node_modules"),
        output: path.join(__dirname, "../../build/test_pr_num"),
        recompile: true,
        verbose: true
      }
    )
  ...
  it("Should generate witness", async () => {
    const input = {
      "msg": textToAsciiArray("Merged #16 into ")
    };
    const expectedValue = Array(textToAsciiArray("Merged #").length).fill("0")
      .concat(textToAsciiArray("16"))
      .concat(Array(textToAsciiArray(" into ").length).fill("0"))
